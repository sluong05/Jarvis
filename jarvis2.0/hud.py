...
def draw(frame, gesture: GestureType, paused: bool, gaze_xy=None):
    h, w = frame.shape[:2]
    ...
    cv2.putText(frame, "JARVIS 2.0", (10, 35),
                cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 200, 255), 2)

    # Gaze indicator
    cv2.putText(frame, "GAZE", (175, 35),
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 200), 2)

    # Hand gesture label (actions only — no CURSOR)
    label = GESTURE_LABELS.get(gesture, "---")
    color = GESTURE_COLORS.get(gesture, (200, 200, 200))
    cv2.putText(frame, label, (280, 35),
